...
def get_reward(conversation, history_file, llm_name):
	llm_count = 0
	...
	for i in range(5):
		...
		save_llm_result(gpt_reply, history_file)
		llm_count += 1
		...
		if gpt_reply_json['result'] == True and 'explain' not in gpt_reply_json:
			gpt_reply_json['explain'] = ''
		return gpt_reply_json['result'], gpt_reply_json['explain'], llm_count
...
class TaskTree:
	def __init__(self, root_task="", root_description="", scene_graph="", generate_task_decompose_data=False, task_decompose_data_save_path="", history_file="results.jsonl"):
		"""
	...
	def process_messages(self, task_node, gpt_reply):
		...
		label, explain, llm_count = get_reward(messages, self.history_file, self.llm_name)
	...
	def split_task(self, task_node):
		...
		while is_success == False:
			...
			for i in range(5):
				...
				save_llm_result(gpt_reply, self.history_file)
			...
			is_success, explain= self.process_messages(task_node, gpt_reply)
			if is_success == False:
				messages.append({'role': 'assistant', 'content': gpt_reply})
				messages.append({'role': 'user', 'content' : f'The above way of decomposing tasks is not good enough. {explain}\
	...
	def build_tree(self):
		...
		while queue:
			...
			if not current_node.is_leaf:
				...
				plan.append(current_node.formated_description)
	...
	def save_for_distill(self, save_path):
		...
		while queue:
			...
			if not current_node.is_leaf:
				count += 1
				decompose_str += f'Among the whole plan, "{current_node.description}" is still an abstract task. I will continue to decompose it:\n'

				sub_task = []
				sub_descriptions = []
				for child in current_node.children:
					sub_descriptions.append(child.description) 
					if child.is_leaf:
				...
				decompose_str += f'It can be decomposed to the following abstract steps: {", ".join(sub_descriptions)}\n'
				decompose_str += f'When written in grounded format: {", ".join(sub_task)}\n'
				try:
				...
				if count == 3:
					count = 0 
					decompose_str += f"Now,  I will review the whole plan: {', '.join(whole_plan)}\n"
